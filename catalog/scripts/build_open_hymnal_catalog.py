...
def _write_web_catalog(
    ...
    *,
    ...
    for item in items:
        ...
        lyricist = display.get("lyricist") or display["text_author"]
        collection_title = display.get("collection") or ""
        ensemble = display.get("ensemble") or "SATB choir"
        available_lines = [
            {
                "SCORE": "score",
                "SATB": "satb",
            ...
            for value in item["available_lines"]
        ]
        lyrics_scope = item["lyrics"]["scope"]
        lyrics_available_for = (
            ["satb", "soprano"]
            if lyrics_scope == "soprano_only"
            else (["score"] if item["lyrics"]["available"] else [])
        )
        lines.extend(
            [
                "  {",
                ...
                f"    contentType: {json.dumps(content_type, ensure_ascii=False)},",
                ...
                f"    composer: {json.dumps(composer, ensure_ascii=False)},",
                ...
                f"    lyricsAvailableFor: {json.dumps(lyrics_available_for, ensure_ascii=False)},",
                f"    catalogRevision: {catalog_revision},",
                f"    scoreSha256: {json.dumps(item['score']['sha256'])},",
                f"    rightsStatus: {json.dumps(item['rights']['status'])},",
    ...
    lines.extend(
        [
            "] as const;",
            "",
            "export const generatedHymns = generatedCatalogItems.filter(",
...
def build_catalog(
    *,
    ...
    catalog = {
        "catalog_id": "transposify-technical-preview",
