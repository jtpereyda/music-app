...
changed. When a preview branch has no previous successful deployment SHA, it
requests a full renderer build instead of attempting an invalid diff, so the
Vercel check remains reliable.
